...
public class BybitServiceImpl extends AbstractExchangeService implements ExchangeService {
  ...
  public List<BalanceDto> fetchBalances(ApiKey key) {
    try {
      ...
      List<Map<String, Object>> rawBalances =
          (List<Map<String, Object>>) ((Map<String, Object>) response.get("result")).get("list");
  ...
  protected List<BalanceDto> parseBalances(List<Map<String, Object>> rawBalances) {
    ...
    for (Map<String, Object> balanceMap : rawBalances) {
      ...
      for (Object c : coins) {
        ...
        BalanceDto dto =
            new BalanceDto(asset, available, locked, walletBalance, usdValue, Exchange.BYBIT);
        if (dto.getTotal() > 0) {
          result.add(dto);
        }
      }
  ...
  public SignedRequest buildSignedRequest(ApiKey apiKey, String path, String payload,
    ...
    try {
      ...
      String preHash =
          (method == HttpMethod.GET) ? timestamp + apiKey.getApiKey() + recvWindow + data
  ...
  public OrderResult placeMarketOrder(ApiKey key, String symbol, double quantity,
    ...
    try {
      ...
      OrderResult result = new OrderResult(true, String.valueOf(data.get("orderId")), symbol,
          Double.parseDouble(data.get("cumExecQty").toString()), avgPrice,
  ...
  private void placePlanOrder(ApiKey key, String symbol, double quantity, Direction direction,
      double triggerPrice, String tpSlType) {
    String path = "/v5/order/create";
    String url = BASE_URL + path;
