...
public abstract class AbstractExchangeService {
	...
	protected BalanceDto toBalanceDto(String asset, double available, double locked, Exchange exchange) {
		double total = available + locked;
		double usdRate = getUsdRate(asset);
		double usdValue = total * usdRate;

		return new BalanceDto(asset, available, locked, total, usdValue, exchange);
	}
	...
	protected ResponseEntity<Map<String, Object>> getWithHeaders(String url, HttpHeaders headers) {
		...
		ResponseEntity<Map<String, Object>> result = restTemplate
			.exchange(URI.create(url), HttpMethod.GET, requestEntity,
