		...
		for (Position position : activePositions) {
			...
			try {
				for (var cond : position.getConditions()) {
					...
					log
						.info("\n🧩 [조건 평가 시작] - 타입: {}, 연산자: {}, 타임프레임: {}", cond.getType(), cond.getOperator(),
								timeframe);
					...
					case RSI -> {
						...
						if (!rsiList.isEmpty()) {
							...
							log.info("📈 [RSI 검사] 현재: {}, 기준: {}, 연산자: {}", currentRsi, value, cond.getOperator());
					...
					case STOCH_RSI -> {
						...
						if (!stochList.isEmpty()) {
							...
							log
								.info("📉 [StochRSI 검사] K: {}, D: {}, 기준: {}, 연산자: {}", currentK, currentD, value,
										cond.getOperator());
							...
							if (kTarget != null && dTarget != null) {
								if (currentK > currentD && currentK - currentD >= 0.5) {
									...
									log
										.info("❌ 교차 조건 실패 (%K={}, %D={}, 차이={})", currentK, currentD,
												currentK - currentD);
									isPass = false;
								}
							}
					...
					case VWBB -> {
						...
						if (!basis.isEmpty()) {
							...
							log
								.info("📊 [VWBB 검사] 현재가: {}, 상단: {}, 기준선: {}, 하단: {}, 캔들 수: {}, 마지막 캔들 UTC: {}",
